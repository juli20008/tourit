...
def _generate_per_photo_narrations(listing_data, photo_labels, active_groups=None, cover_lines=None):
    ...
    label_lines = "\n".join(f"  第{i+1}张=【{lbl}】→只能写{lbl}，禁止写其他任何房间" for i, lbl in enumerate(photo_labels))
...
【铁律】每张照片的房间标注是用户亲自确认过的，必须严格遵守，不得根据房源信息自行推断或更改。标注说是厨房就写厨房，标注说是主卧就写主卧，哪怕你觉得应该写别的也不行。

每句话规则：
- 约12字（10-14字之间，绝不超过15字）
- 严格按下方标注的房间写，绝不能写其他房间
- 连续同一房间多张照片：只在该房间的第一张提房间名，后续几张直接描述细节，不要重复说房间名。每张从不同角度切入：①整体空间感②采光/窗户③某个具体细节（材质/家具/工艺）④住起来的感觉⑤与其他空间的关系
- 第1张必须自然融入基本规格（几室几卫、面积）
- 语气积极阳光像朋友聊天，禁止「不是X而是Y」「并非」「别小看」等否定句式
- 只说listing里有的内容，严禁编造
- 严禁提地址、门牌号、价格、挂牌价，一个字也不能有

{floor_context}【照片房间标注 — 必须严格遵守】：
{label_lines}

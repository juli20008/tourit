...
(() => {
  ...
  async function startFill(listing) {
    ...
    if (imageUrls.length) {
      ...
      if (imageFiles.length === 0) {
        ...
        showStatus(`Fetched ${imageFiles.length}/${imageUrls.length} image(s). Generating QR slide…`, 'info');
      } else {
        showStatus(`Got ${imageFiles.length} image(s). Generating QR slide…`, 'info');
      }
    } else {
      showStatus('No images in listing. Generating QR slide…', 'info');
    }

    // ── QR code slide — appended as the last photo ─────────────────────────
    try {
      const qrSlide = await generateQrSlide(listing);
      if (qrSlide) {
        imageFiles = [...imageFiles, qrSlide];
      } else {
        console.warn('[Tourit FBMP] QR slide generation failed, uploading without QR');
      }
    } catch (e) {
      console.warn('[Tourit FBMP] QR slide error:', e.message);
    }
  ...
  // ─── QR code slide (last photo, English) ─────────────────────────────────

  async function generateQrSlide(listing) {
    const mls = listing.mls_number || '';

    // Respect agent whitelabel URL if available
    ...
    // Load logo — must come from extension bytes to avoid canvas cross-origin taint
    let logoImg = null;
    if (logoResp.data) {
    ...
    return new Promise(resolve => {
      ...
      // Logo — target ~28% of canvas height (1/4–1/3 range)
      let logoYEnd = 20;
      if (logoImg) {
        const targetH = 300;
        const maxW    = Math.round(size * 0.82);
        ...
        ctx.drawImage(logoImg, Math.round((size - logoW) / 2), 20, logoW, logoH);
        logoYEnd = 20 + logoH;
      }
      ...
      ctx.fillText('Scan to Book a Showing', size / 2, logoYEnd + 70);
  ...
  async function autofillListing(listing) {
    ...
    await fillTextField(['description'], buildDescription(listing), filled, skipped, 'Description', { multiline: true });
  ...
  function buildDescription(listing) {
    const base = String(listing.description || '').trim();
    const url  = listing.mls_number
      ? `https://tourit.ca/listing/${listing.mls_number}`
      : 'https://tourit.ca';
    const line = `Schedule a showing: ${url}`;
    return base ? `${base}\n\n${line}` : line;
  }
